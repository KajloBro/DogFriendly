...
$small = ("SELECT p.id, p.path, p.path_big, p.part, a.name FROM accommodation_pics AS p JOIN "
        . "accommodation AS a ON p.fk_accomm = a.id WHERE size = 'small' AND p.fk_accomm = $id");
...
?>
    ...
    echo '<div class="col-md-3 col-sm-6 col-xs-12">';
        echo '<button type="button" class="btn btn-info btn_but_modal btn-lg btn_padding" data-toggle="modal" data-target="#myModal'.$pic_id.'">'
                . '<img src="'.$row_s['path'].'" alt="'.$row_s['part'].'" class="modal_small"/></button>';
            ...
            echo '<div class="modal-dialog modal-lg">';
                echo '<div class="modal-content">';
                    ...
                    echo '<div class="modal-body">';
                        echo '<img src="'.$row_s['path_big'].'" alt="'.$row_s['part'].'" class="modal_small"/>';
                    echo '</div>';
                echo '</div>';
            echo '</div>';
        echo '</div>';
